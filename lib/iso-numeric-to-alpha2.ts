/**
 * Compact mapping from ISO 3166-1 numeric (the M49 codes used by
 * world-atlas / Natural Earth topojson) to alpha-2 codes (what the rest
 * of the app stores in `place.country_code`).
 *
 * Source: https://en.wikipedia.org/wiki/ISO_3166-1_numeric — current
 * country list, no reserved/transitional codes.
 */
export const ISO_NUMERIC_TO_ALPHA2: Record<string, string> = {
  ...
  // Special: world-atlas uses "-99" for unrecognised territories
  // (Kosovo, N. Cyprus etc.). Mapping skipped intentionally.
};

export function alpha2FromNumeric(numericId: string | number): string | null {
  const key =
    typeof numericId === "number"
      ? String(numericId).padStart(3, "0")
      : numericId.padStart(3, "0");
